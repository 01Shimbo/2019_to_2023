def main():
    ...
    # element_data = make_periodic_table()
    # for element in element_data:
    #     print(element[NAME_INDEX], element[ATOMIC_MASS_INDEX])
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    element_keys = periodic_table_dict.keys()
    molar_mass = 0.00
    for element, quantity in symbol_quantity_list:
        if element in element_keys:
            element = periodic_table_dict[element]
            molar_mass += element[ATOMIC_MASS_INDEX] * quantity
